...
FILES_PER_ENTITY = int(Path('.env').read_text().split('OPENALEX_DEMO_FILES_PER_ENTITY=')[1].split('\n')[0]) if Path('.env').exists() else 0

csv_files = {
    'authors': {
        'authors': {
            'name': Path(str(CSV_DIR)+'/authors.csv.gz'),
            'columns': [
        ...
        'ids': {
            'name': Path(str(CSV_DIR)+'/authors_ids.csv.gz'),
            'columns': [
                'author_id', 'openalex', 'orcid', 'scopus', 'twitter', 'wikipedia', 'mag'
            ]
        },
        'counts_by_year': {
            'name': Path(str(CSV_DIR)+'/authors_counts_by_year.csv.gz'),
            'columns': [
    ...
    'concepts': {
        'concepts': {
            'name': Path(str(CSV_DIR)+'/concepts.csv.gz'),
            'columns': [
        ...
        'ancestors': {
            'name': Path(str(CSV_DIR)+'/concepts_ancestors.csv.gz'),
            'columns': ['concept_id', 'ancestor_id']
        },
        'counts_by_year': {
            'name': Path(str(CSV_DIR)+'/concepts_counts_by_year.csv.gz'),
            'columns': ['concept_id', 'year', 'works_count', 'cited_by_count', 'oa_works_count']
        },
        'ids': {
            'name': Path(str(CSV_DIR)+'/concepts_ids.csv.gz'),
            'columns': ['concept_id', 'openalex', 'wikidata', 'wikipedia', 'umls_aui', 'umls_cui', 'mag']
        },
        'related_concepts': {
            'name': Path(str(CSV_DIR)+'/concepts_related_concepts.csv.gz'),
            'columns': ['concept_id', 'related_concept_id', 'score']
        }
    },
    'institutions': {
        'institutions': {
            'name': Path(str(CSV_DIR)+'/institutions.csv.gz'),
            'columns': [
        ...
        'ids': {
            'name': Path(str(CSV_DIR)+'/institutions_ids.csv.gz'),
            'columns': [
                'institution_id', 'openalex', 'ror', 'grid', 'wikipedia', 'wikidata', 'mag'
            ]
        },
        'geo': {
            'name': Path(str(CSV_DIR)+'/institutions_geo.csv.gz'),
            'columns': [
        ...
        'associated_institutions': {
            'name': Path(str(CSV_DIR)+'/institutions_associated_institutions.csv.gz'),
            'columns': [
                'institution_id', 'associated_institution_id', 'relationship'
            ]
        },
        'counts_by_year': {
            'name': Path(str(CSV_DIR)+'/institutions_counts_by_year.csv.gz'),
            'columns': [
    ...
    'publishers': {
        'publishers': {
            'name': Path(str(CSV_DIR)+'/publishers.csv.gz'),
            'columns': [
        ...
        'counts_by_year': {
            'name': Path(str(CSV_DIR)+'/publishers_counts_by_year.csv.gz'),
            'columns': ['publisher_id', 'year', 'works_count', 'cited_by_count', 'oa_works_count']
        },
        'ids': {
            'name': Path(str(CSV_DIR)+'/publishers_ids.csv.gz'),
            'columns': ['publisher_id', 'openalex', 'ror', 'wikidata']
        },
    },
    'sources': {
        'sources': {
            'name': Path(str(CSV_DIR)+'/sources.csv.gz'),
            'columns': [
        ...
        'ids': {
            'name': Path(str(CSV_DIR)+'/sources_ids.csv.gz'),
            'columns': ['source_id', 'openalex', 'issn_l', 'issn', 'mag', 'wikidata', 'fatcat']
        },
        'counts_by_year': {
            'name': Path(str(CSV_DIR)+'/sources_counts_by_year.csv.gz'),
            'columns': ['source_id', 'year', 'works_count', 'cited_by_count', 'oa_works_count']
        },
    },
    'works': {
        'works': {
            'name': Path(str(CSV_DIR)+'/works.csv.gz'),
            'columns': [
        ...
        'primary_locations': {
            'name': Path(str(CSV_DIR)+'/works_primary_locations.csv.gz'),
            'columns': [
                'work_id', 'source_id', 'landing_page_url', 'pdf_url', 'is_oa', 'version', 'license'
            ]
        },
        'locations': {
            'name': Path(str(CSV_DIR)+'/works_locations.csv.gz'),
            'columns': [
                'work_id', 'source_id', 'landing_page_url', 'pdf_url', 'is_oa', 'version', 'license'
            ]
        },
        'best_oa_locations': {
            'name': Path(str(CSV_DIR)+'/works_best_oa_locations.csv.gz'),
            'columns': [
                'work_id', 'source_id', 'landing_page_url', 'pdf_url', 'is_oa', 'version', 'license'
            ]
        },
        'authorships': {
            'name': Path(str(CSV_DIR)+'/works_authorships.csv.gz'),
            'columns': [
                'work_id', 'author_position', 'author_id', 'institution_id', 'raw_affiliation_string'
            ]
        },
        'biblio': {
            'name': Path(str(CSV_DIR)+'/works_biblio.csv.gz'),
            'columns': [
                'work_id', 'volume', 'issue', 'first_page', 'last_page'
            ]
        },
        'concepts': {
            'name': Path(str(CSV_DIR)+'/works_concepts.csv.gz'),
            'columns': [
                'work_id', 'concept_id', 'score'
            ]
        },
        'ids': {
            'name': Path(str(CSV_DIR)+'/works_ids.csv.gz'),
            'columns': [
                'work_id', 'openalex', 'doi', 'mag', 'pmid', 'pmcid'
            ]
        },
        'mesh': {
            'name': Path(str(CSV_DIR)+'/works_mesh.csv.gz'),
            'columns': [
                'work_id', 'descriptor_ui', 'descriptor_name', 'qualifier_ui', 'qualifier_name', 'is_major_topic'
            ]
        },
        'open_access': {
            'name': Path(str(CSV_DIR)+'/works_open_access.csv.gz'),
            'columns': [
                'work_id', 'is_oa', 'oa_status', 'oa_url', 'any_repository_has_fulltext'
            ]
        },
        'referenced_works': {
            'name': Path(str(CSV_DIR)+'/works_referenced_works.csv.gz'),
            'columns': [
                'work_id', 'referenced_work_id'
            ]
        },
        'related_works': {
            'name': Path(str(CSV_DIR)+'/works_related_works.csv.gz'),
            'columns': [
...
def flatten_authors():
    ...
    with gzip.open(file_spec['authors']['name'], 'wt', encoding='utf-8') as authors_csv, \
        ...
        total_size_done = 0
        total_time = 0
        files = sorted(Path(SNAPSHOT_DIR).glob('data/authors/*/*.gz')) 
        total_size = sum(f.stat().st_size for f in files)
        for jsonl_file_name in files:
            print(files_done, jsonl_file_name)
            start_time = time.time()
            with gzip.open(jsonl_file_name, 'rt') as authors_jsonl:
                for author_json in filter(str.strip,authors_jsonl):
            ...
            files_done += 1  
            total_size_done += Path(jsonl_file_name).stat().st_size 
...
def flatten_concepts():
    with gzip.open(csv_files['concepts']['concepts']['name'], 'wt', encoding='utf-8') as concepts_csv, \
        ...
        files_done = 0
        for jsonl_file_name in sorted(Path(SNAPSHOT_DIR).glob('data/concepts/*/*.gz')): 
            print(jsonl_file_name)
            with gzip.open(jsonl_file_name, 'rt') as concepts_jsonl:
                for concept_json in filter(str.strip, concepts_jsonl):
...
def flatten_institutions():
    ...
    with gzip.open(file_spec['institutions']['name'], 'wt', encoding='utf-8') as institutions_csv, \
        ...
        files_done = 0
        for jsonl_file_name in sorted(Path(SNAPSHOT_DIR).glob('data/institutions/*/*.gz')):
            print(jsonl_file_name)
            with gzip.open(jsonl_file_name, 'rt') as institutions_jsonl:
                for institution_json in filter(str.strip, institutions_jsonl):
                    institution = json.loads(institution_json)
                    institution_id = institution.get('id')

                    if institution_id not in seen_institution_ids:
...
def flatten_publishers():
    with gzip.open(csv_files['publishers']['publishers']['name'], 'wt', encoding='utf-8') as publishers_csv, \
        ...
        files_done = 0
        for jsonl_file_name in sorted(Path(SNAPSHOT_DIR).glob('data/publishers/*/*.gz')):
            print(jsonl_file_name)
            with gzip.open(jsonl_file_name, 'rt') as concepts_jsonl:
                for publisher_json in filter(str.strip, concepts_jsonl):
                    publisher = json.loads(publisher_json)
                    publisher_id = publisher.get('id')
                    if publisher_id not in seen_publisher_ids:
...
def flatten_sources():
    with gzip.open(csv_files['sources']['sources']['name'], 'wt', encoding='utf-8') as sources_csv, \
        ...
        files_done = 0 
        for jsonl_file_name in sorted(Path(SNAPSHOT_DIR).glob('data/resources/*/*.gz')):
            print(jsonl_file_name)
            with gzip.open(jsonl_file_name, 'rt') as sources_jsonl:
                for source_json in filter(str.strip, sources_jsonl):
...
def flatten_works():
    ...
    with gzip.open(file_spec['works']['name'], 'wt', encoding='utf-8') as works_csv, \
        ...
        total_size_done = 0
        total_time = 0
        files = sorted(Path(SNAPSHOT_DIR).glob('data/works/*/*.gz'))
        total_size = sum(f.stat().st_size for f in files)
        for jsonl_file_name in files:
            print(files_done, jsonl_file_name)
            start_time = time.time()
            with gzip.open(jsonl_file_name, 'rt') as works_jsonl:
                for work_json in filter(str.strip, works_jsonl):
                    work = json.loads(work_json)
                    if work_id := work.get('id'):
                    #works
